...
if(isset($_POST['login_button'])) {
	...
	$check_admin1_query = mysqli_query($con, "SELECT * FROM librarians WHERE email='$email' AND password='$password' AND $user_type='1'");
	$check_admin2_query = mysqli_query($con, "SELECT * FROM librarians WHERE email='$email' AND password='$password' AND $user_type='2'");
	...
	if($check_login_query1 == 1) {
		...
		$row = mysqli_fetch_array($check_admin1_query);
		$email = $row['email'];
